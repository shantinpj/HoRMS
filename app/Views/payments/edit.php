<h1 class="text-3xl font-bold mb-8">Edit Rent Payment</h1>

<div class="bg-white rounded-lg shadow-md p-6 max-w-4xl mx-auto">
    <form action="/payments/update" method="POST" id="paymentForm">
        ...
        <div class="mb-6">
            ...
            <select class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="lease_id" name="lease_id" required>
                <?php foreach ($leases as $lease): ?>
                    <option value="<?php echo $lease['id']; ?>" data-rate="<?php echo $lease['electricity_rate']; ?>" <?php echo $lease['id'] == $payment['lease_id'] ? 'selected' : ''; ?>>
        ...
        <div class="mb-6">
            ...
            <input class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="amount" name="amount" type="number" step="0.01" value="<?php echo $payment['base_rent']; ?>" required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <div class="mb-3">
                    ...
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="electricity_units" id="elec_qty" type="number" step="0.01" value="<?php echo $payment['electricity_units']; ?>">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-blue-600 uppercase mb-1">Rate (Rs./Unit)</label>
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="electricity_rate" id="elec_rate" type="number" step="0.01" value="<?php echo $payment['electricity_rate']; ?>">
                </div>
            </div>
            ...
            <div class="bg-green-50 p-4 rounded-lg border border-green-100">
                ...
                <div class="mb-3">
                    ...
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="cleaning_qty" id="clean_qty" type="number" step="0.01" value="<?php echo $payment['cleaning_qty']; ?>">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-green-600 uppercase mb-1">Rate (Rs./Qty)</label>
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="cleaning_rate" id="clean_rate" type="number" step="0.01" value="<?php echo $payment['cleaning_rate']; ?>">
                </div>
            </div>
            ...
            <div class="bg-teal-50 p-4 rounded-lg border border-teal-100">
                ...
                <div class="mb-3">
                    ...
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="water_qty" id="water_qty" type="number" step="0.01" value="<?php echo $payment['water_qty']; ?>">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-teal-600 uppercase mb-1">Rate (Rs./Qty)</label>
                    <input class="shadow border rounded w-full py-1 px-2 text-sm" name="water_rate" id="water_rate" type="number" step="0.01" value="<?php echo $payment['water_rate']; ?>">
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-800 text-white p-4 rounded-lg mb-8 flex justify-between items-center shadow-inner">
            ...
            <span class="text-2xl font-bold text-yellow-400" id="grand_total_display">Rs. <?php echo number_format($payment['amount'], 2); ?></span>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="payment_date">Payment Date</label>
                <input class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="payment_date" name="payment_date" type="date" value="<?php echo $payment['payment_date']; ?>" required>
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="transaction_id">Transaction ID / Reference (Optional)</label>
                <input class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="transaction_id" name="transaction_id" type="text" value="<?php echo htmlspecialchars($payment['transaction_id']); ?>">
            </div>
        </div>

        <div class="flex items-center justify-between border-t pt-6">
            <button class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg focus:outline-none focus:shadow-outline transition duration-200 shadow-md" type="submit">
